...
class SlideshowComponent extends SliderComponent {
  ...
  onButtonClick(event) {
    ...
    // Wrap-around: skip super to avoid conflicting scroll
    if (isFirstSlide && event.currentTarget.name === "previous") {
      event.preventDefault();
      this.wasClicked = true;
      this.wrapSlide("previous");
  ...
  wrapSlide(direction) {
    ...
    const slideWidth = this.sliderFirstItemNode.clientWidth;
    const totalSlides = this.sliderItemsToShow.length;

    const prepareClone = (sourceSlide) => {
      const clone = sourceSlide.cloneNode(true);
      clone.classList.add("slideshow__clone");
      // Give clone a temp id and copy the overlay opacity style
      const origId = sourceSlide.id;
      const cloneId = origId + "-clone";
      clone.id = cloneId;
    ...
    if (direction === "next") {
      const { clone } = prepareClone(this.sliderItemsToShow[0]);
      this.slider.style.scrollSnapType = "none";
      this.slider.appendChild(clone);

      // Immediately update dots to target slide
      this.currentPage = 1;
      this.updateDots();

      this.slider.scrollTo({
        left: this.slider.scrollLeft + slideWidth,
        behavior: "smooth",
      });
      ...
      this.slider.addEventListener("scrollend", onEnd, { once: true });
      const fallbackTimer = setTimeout(() => {
        this.slider.removeEventListener("scrollend", onEnd);
        cleanup();
      }, 800);
    } else {
      const lastSlide = this.sliderItemsToShow[totalSlides - 1];
      const { clone } = prepareClone(lastSlide);
  ...
  setSlidePosition(position, behavior) {
    if (this.setPositionTimeout) clearTimeout(this.setPositionTimeout);
    this.setPositionTimeout = setTimeout(() => {
      const scrollOpts = { left: position };
      if (behavior) scrollOpts.behavior = behavior;
      this.slider.scrollTo(scrollOpts);
  ...
  updateDots() {
    if (!this.sliderControlButtons || !this.sliderControlButtons.length) return;
